...
/*   Updated: 2018/03/31 17:29:06 by bacrozat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		error_instr(char cse, int line, char *instr)
{
	...
	if (cse == 3)
		ft_printf("%rgbNumber of SEPARATOR_CHAR did not match the expected number of parameters:\n%s\t<- line %d\n%0rgb", RED, instr, line);
	if (cse == 4)
		ft_printf("%rgbNumber of written parameter does not match the number of actual parameters:\n%s\t<- line %d\n%0rgb", RED, instr, line);
	if (cse == 5)
		ft_printf("%rgbUnknown instruction in expression:\n%s\t<- line\n%0rgb", RED,instr, line);
	if (cse == 6)
		ft_printf("%rgbA parameter contains an unexpected cararcter: %c\n%0rgb", RED, *instr);
	if (cse == 7)
		ft_printf("%rgbLine contains and extra SEPARATOR_CHAR with no following parameter.\n%0rgb", RED);
	if (cse == 7)
