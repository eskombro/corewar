...
static void			player_name(t_visu *visu, t_player *player,
						t_player_info *pinfo, t_main_scene *main)
{
	if (!(pinfo->name = sg_create_label(main->ptr)))
		error(ERR_MALLOC, ERR_CRITICAL);
	sg_set_component_boundaries(pinfo->name,
		sg_recti(visu->win_w - 2 * MAIN_SCENE_MARGIN,
			visu->win_h - (2 + player->visu_id) * MAIN_SCENE_MARGIN -
			MAIN_SCENE_TOP_HEIGHT - (int)(((player->visu_id - 1) * 2.5) *
...
static void			player_id(t_visu *visu, t_player *player,
						t_player_info *pinfo, t_main_scene *main)
{
	char			*tmp;
	...
	sg_set_component_boundaries(pinfo->id,
		sg_recti(visu->win_w - (int)(2.5 * MAIN_SCENE_MARGIN),
			visu->win_h - (2 + player->visu_id) * MAIN_SCENE_MARGIN -
			MAIN_SCENE_TOP_HEIGHT - (int)(((player->visu_id) * 2.5 - 1.5) *
				COREWAR_TOP_FONT_SIZE), 0, 0));
						...
						t_player_info *pinfo, t_main_scene *main)
{
	...
	sg_set_component_boundaries(pinfo->alive,
		sg_recti(visu->win_w - (int)(2.5 * MAIN_SCENE_MARGIN),
			visu->win_h - (2 + player->visu_id) * MAIN_SCENE_MARGIN -
			MAIN_SCENE_TOP_HEIGHT - (int)(((player->visu_id) * 2.5 - 0.75) *
				COREWAR_TOP_FONT_SIZE), 0, 0));
...
void				init_player_info_gui(t_visu *visu, t_player *player)
{
	...
	pinfo->player = player;
	player_name(visu, player, pinfo, main);
	player_id(visu, player, pinfo, main);
	player_alive(visu, player, pinfo, main);
